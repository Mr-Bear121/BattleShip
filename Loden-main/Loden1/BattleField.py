...
from BattleShipStart import hotSeatGame,lanGame,returnWindowData
...
class GridT(aGrid):
    ...
    def __init__(self,params=GridParams()):
        ...
        self._times = 0
        self.zpane = 'testing no value'
    
    def _init_win(self):
        self._win = tk.Toplevel(self.params.hostWindow)
        self.params.rData = tk.StringVar()
        self._win.title(f"{self.params.player.commanderName} {self.params.gridType}")
        ...
        for yhigh in range(self.params.cells_wide):
            for xwide in range(self.params.cells_high):
                a_wgt = tk.Button(
                    ...
                    relief=tk.SOLID,
                    #in order to use this properly you should use a lambda or else it will automatically execute
                    command=lambda value=buttonPosition:self.params.gameType(self.params.player,self.params.gridType,value,self.params.rules)
                    )
                buttonPosition+=1
    ...
    def getCell(self,value):
        return self._cells[value]

    def returnData(self,data):
        returnWindowData(data)

    def hideWindow(self,window):
        return window.withdraw()
    ...
    def windowAction(self, action):
        if action == 'hide':
            self.hideWindow(self._win)
        elif action == 'show':
            self.showWindow(self._win)
        return

    def updateZPane(self,pain):
        self.zpane.data=pain
        return 'updated'
...
#    params.font_high = 1
#    params.font_size = 28
    #params.color_fore="#ffffff"
#    w = GridT.Create(params)
    #print(w.getBtn())
    #w.placeShip(w.getBoard(),w.getBtn())
